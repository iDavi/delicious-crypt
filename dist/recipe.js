...
const span = (start, count, step = 1) => Array.from({ length: count }, (_, i) => start + i * step);
// "tsp" is the only unit written differently across languages.
const unitText = (u, lang) => (u === "tsp" && lang === "pt" ? "c. chá" : u);
const INGREDIENTS = [
    // For the cake
...
const T = {
    en: {
        ...
        title: (adj) => `# ${adj} Layer Cake`,
        meta: (s, p, c, b, t) => `Serves ${s}. Prep ${p} min, chill ${c} min, bake ${b} min at ${t}°C.`,
        ingHead: "## Ingredients",
        methodHead: "## Method",
        method: [
    ...
    pt: {
        ...
        bake: [25, 30, 35, 40, 45],
        temp: [160, 170, 180, 190],
        title: (adj) => `# Bolo de Camadas ${adj}`,
        meta: (s, p, c, b, t) => `Rende ${s}. Preparo ${p} min, geladeira ${c} min, forno ${b} min a ${t}°C.`,
...
// --- render -----------------------------------------------------------------
export function toRecipe(bytes, lang) {
    ...
    const out = [t.title(adj), "", tag, size, "", t.meta(serves, prep, chill, bake, temp), "", t.ingHead];
    let ingIdx = 0;
...
// --- parse ------------------------------------------------------------------
function parseDigits(text, lang) {
    const t = T[lang];
    const lines = text.split("\n").map((l) => l.trim());
    const digits = [];
    const idx = (arr, v, what) => {
        const i = arr.findIndex((x) => String(x) === String(v));
        if (i < 0)
            throw new Error(`unreadable ${what}`);
        return i;
    };
    ...
    if (ingLines.length !== INGREDIENTS.length)
        throw new Error("wrong ingredient count");
    INGREDIENTS.forEach((g, i) => {
        const m = ingLines[i].slice(2).match(/^(\d+)\s+(.*)$/);
        if (!m)
            throw new Error("unreadable ingredient");
        let rest = m[2];
        const u = unitText(g.unit, lang);
        if (u) {
            if (!rest.startsWith(u + " "))
                throw new Error("unreadable unit");
    ...
    if (methodLines.length !== t.method.length)
        throw new Error("wrong step count");
    t.method.forEach((variants, i) => digits.push(idx(variants, methodLines[i], "method step")));
    return digits;
}
export function fromRecipe(text) {
...
export function toMessage(bytes) { return dec.decode(bytes); }
export function fromMessage(s) { return enc.encode(s); }
